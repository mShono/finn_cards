...
def _chat_instructions(*, is_follow_up: bool) -> str:
    ...
    text = (
        ...
        "напиши только что-то вроде «Добавляю» - **никогда не пиши, что "
        "слова уже сохранены, и никогда не называй колоду**: сохранение и "
        "выбор колоды - отдельный шаг нашего кода уже после этого ответа "
        "(ученик сам выбирает или называет колоду через /decks), тебе "
        "результат неизвестен и колоду ты не выбираешь и не придумываешь "
        "никогда. `needs_clarification: false`. В `candidates` - карточки "
